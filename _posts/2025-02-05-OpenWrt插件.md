...
| luci-app-openclash | 一个运行在 OpenWrt 上的 Clash 客户端，兼容 Shadowsocks(R)、Vmess、Trojan、Snell 等协议，根据灵活的规则配置实现策略代理,需要nohup,可能需要重安装coreutils-nohup |
...
| luci-app-syncdial |  |
| luci-app-sysuh3c |  |
| luci-app-unblockneteasemusic | 解除网易云音乐播放限制，使用后不能正常访问 |
| luci-app-xfrpc |  |
| luci-app-xinetd |  |
...
| luci-i18n-battstatus-zh-cn | 在LuCI中提供电池电量指示器。目前仅支持HooToo HT-TM05旅行路由器。 |
...
| luci-i18n-kcptun-zh-cn | 基于KCP协议的网络加速工具,可以有效提高数据传输速度 |
...
| luci-i18n-n2n-zh-cn | 内网穿透 |
| luci-i18n-natmap-zh-cn | 实现自由的端口映射工具 |
| luci-i18n-netdata-zh-cn | 性能监控 |
| luci-i18n-nextdns-zh-cn |  |
| luci-i18n-nfs-zh-cn | NFS 管理 |
| luci-i18n-nft-qos-zh-cn |  |
| luci-i18n-ngrokc-zh-cn | 实现内网穿透，允许外部用户访问本地服务 |
| luci-i18n-nlbwmon-zh-cn |  |
| luci-i18n-nps-zh-cn | 一款轻量级、高性能、功能强大的内网穿透代理服务器。 |
| luci-i18n-nut-zh-cn |  |
...
| luci-i18n-passwall-zh-cn | 一款多功能脚本界面程序,支持配合各类流行的代理服务器程序进行使用,不管使用何种代理服务器,都可以通过这个界面程序进行统一控制 |
| luci-i18n-pbr-zh-cn |  |
...
| luci-i18n-sqm-zh-cn | 智能队列管理 |
| luci-i18n-squid-zh-cn |  |
| luci-i18n-ssr-libev-server-zh-cn |  |
| luci-i18n-syncthing-zh-cn | syncthing同步工具 |
| luci-i18n-timewol-zh-cn | 定时唤醒你的局域网设备，可使用定时加wol模块实现 |
...
| luci-i18n-unbound-zh-cn | 高性能的、支持递归解析的DNS服务器 |
