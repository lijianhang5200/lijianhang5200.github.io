...
| curm-firstday | 兼容历史数据,不推荐使用:当月一号(如：2014-08-01) | ${taskid?calDate(0,'M','yyyy-MM')}-01 |
...
| l2d-taskid | 兼容历史数据,不推荐使用:当前批次前2天(YYYY-MM-DD) | ${taskid?calDate(-2,'D','yyyy-MM-dd')} |
...
| l3d-taskid | 兼容历史数据,不推荐使用:日批次前3天（yyyy-mm-dd） | ${taskid?calDate(-3,'D','yyyy-MM-dd')} |
| l3d_taskid | 日批次前3天（yyyymmdd） | ${taskid?calDate(-3,'D','yyyyMMdd')} |
| l3h_hour_id | 前3小时 | ${taskid?calDate(-3,'h','yyyyMMddHH')} |
| l3m-taskid | 兼容历史数据,不推荐使用:三月前当天（yyyy-mm-dd） | ${taskid?calDate(-3,'M','yyyy-MM-dd')} |
...
| ld-taskid | 兼容历史数据,不推荐使用:日批次前一天（YYYY-MM-DD） | ${taskid?calDate(-1,'D','yyyy-MM-dd')} |
...
| lm-01taskid | 兼容历史数据,不推荐使用:上月1号（yyyy-mm -01） | ${taskid?calDate(-1,'M','yyyy-MM')}-01 |
| lm-mtaskid | 兼容历史数据,不推荐使用:上月批次(yyyy-mm) | ${taskid?calDate(-1,'M','yyyy-MM')} |
| lm-taskid | 兼容历史数据,不推荐使用:日批次对应上月同期日（yyyy-mm-dd） | ${taskid?calDate(-1,'M','yyyy-MM-dd')} |
...
| ly-mtaskid | 兼容历史数据,不推荐使用:去年同月（yyyy-mm） | ${taskid?calDate(-1,'Y','yyyy-MM')} |
| ly-taskid | 兼容历史数据,不推荐使用:去年同期日（yyyy-mm-dd) | ${taskid?calDate(-1,'Y','yyyy-MM-dd')} |
...
| lyld-taskid | 兼容历史数据,不推荐使用:去年同期前一天（yyyy-mm-dd） | ${taskid?calDate(-1,'Y')?calDate(-1,'D','yyyy-MM-dd')} |
...
| m-taskid | 兼容历史数据,不推荐使用:月批次（yyyy-mm） | ${taskid?calDate(0,'M','yyyy-MM')} |
...
| nd-taskid | 兼容历史数据,不推荐使用:日批次对应后一天（yyyy-mm-dd） | ${taskid?calDate(+1,'D','yyyy-MM-dd')} |
...
| nm-01taskid | 兼容历史数据,不推荐使用:下月1号（yyyy-mm -01） | ${taskid?calDate(+1,'M','yyyy-MM')}-01 |
...
| task-id | 兼容历史数据,不推荐使用:当前批次日期(yyyy-mm-dd) | ${taskid?calDate(+0,'D','yyyy-MM-dd')} |
